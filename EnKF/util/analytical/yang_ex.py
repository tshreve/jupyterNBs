...
def yang_disp_pool(enkf_x,enkf_y,RP):
    # calculate the model
    return yang_disp(enkf_x, enkf_y, 0,
                     xs=RP[0]*1e3, ys=RP[1]*1e3, zs=-RP[2]*1e3,
                     a=RP[3]*1e3, b=RP[4]*RP[3]*1e3, theta=RP[5], phi=RP[6], 
                     dP=RP[7]*1e6,E=30e9, nu=0.25)
...
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
def yang_disp(x, y, z,
              xs=0, ys=0, zs=5e3, a=1000, b=300, dP=10e6,
              theta=np.pi, phi=0, E=30e9, nu=0.25
              ):
    ...
    a1, b1, csi, Pdila, Pstar = yangpar(a, b, lamda, mu, nu, dP)
...
def yangpar(a, b, lamda, mu, nu, P):
    ...
    c = np.sqrt(a ** 2 - b ** 2, dtype=complex)
    # for convenience
    a2 = a ** 2
    a3 = a ** 3
    b2 = b ** 2
    ...
    R = (1 - 2 * nu) / den1
    # param from (1) [dimensionless]
    Ia = -coef1 * (2 / (a * c2) + np.log(ac) / c3)
    # param from (1) [1/m^2]
    Iaa = -coef1 * (2 / (3 * a3 * c2) + 2 / (a * c4) + np.log(ac) / c5)
