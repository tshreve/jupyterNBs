...
#--------------------------------------------------
# Displacements only
def displacement(xs, ys, zs, xc, yc, zc, a, A, dip, strike, DP, mu, nu=0.25):
    '''
...
#--------------------------------------------------
# Strain only
def runYang_strain(xs, ys, zs, xc, yc, zc, a, A, dip, strike, DP, mu, nu=0.25):
    '''
    ...
    Returns:
            * eea, gamma1, gamma2        : areal and shear strains
    '''
    ...
    h = 0.001*np.abs(max(xs)-min(xs))

    #Displacement gradient tensor
    Ux_p1, Uy_p1, Uz_p1 = runYang_disp(xs+h, ys, zs, xc, yc, zc, a, b, dip, strike, P, mu, nu, lambd)
    Ux_m1, Uy_m1, Uz_m1 = runYang_disp(xs-h, ys, zs, xc, yc, zc, a, b, dip, strike, P, mu, nu, lambd)
    dudx = 0.5 * (Ux_p1 - Ux_m1)/h
    dvdx = 0.5 * (Uy_p1 - Uy_m1)/h

    Ux_p2, Uy_p2, Uz_p2 = runYang_disp(xs, ys+h, zs, xc, yc, zc, a, b, dip, strike, P, mu, nu, lambd)
    Ux_m2, Uy_m2, Uz_m2 = runYang_disp(xs, ys-h, zs, xc, yc, zc, a, b, dip, strike, P, mu, nu, lambd)
    dudy = 0.5 * (Ux_p2 - Ux_m2)/h  
    dvdy = 0.5 * (Uy_p2 - Uy_m2)/h
...
#--------------------------------------------------
# Tilt only
def runYang_tilt(xs, ys, zs, xc, yc, zc, a, A, dip, strike, DP, mu, nu=0.25):
    '''
    Yang formulation adapted from dMODELS.
    Maurizio Battaglia, et al, dMODELS: A MATLAB software package for modeling crustal deformation near active faults and volcanic centers, JVGR, Volume 254, 2013.
    ...
    Returns:
       * dwdx, dwdy        : east and north components of tilt
    '''
    ...
    Ux_p1, Uy_p1, Uz_p1 = runYang_disp(xs+h, ys, zs, xc, yc, zc, a, b, dip, strike, P, mu, nu, lambd)
    Ux_m1, Uy_m1, Uz_m1 = runYang_disp(xs-h, ys, zs, xc, yc, zc, a, b, dip, strike, P, mu, nu, lambd)
    dwdx = 0.5 * (Uz_p1 - Uz_m1)/h

    #North component
    Ux_p2, Uy_p2, Uz_p2 = runYang_disp(xs, ys+h, zs, xc, yc, zc, a, b, dip, strike, P, mu, nu, lambd)
    Ux_m2, Uy_m2, Uz_m2 = runYang_disp(xs, ys-h, zs, xc, yc, zc, a, b, dip, strike, P, mu, nu, lambd)
    dwdy = 0.5 * (Uz_p2 - Uz_m2)/h
